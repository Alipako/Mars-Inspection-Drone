...
void detection_cb(const darknet_ros_msgs::BoundingBoxes::ConstPtr& msg)
{
	for( int i=0; i < msg->bounding_boxes.size(); i++)
	{
		...
		if(msg->bounding_boxes[i].Class == "cave")
		{
			...
			aria = (msg->bounding_boxes[i].xmax - msg->bounding_boxes[i].xmin)*(2*tan(1.272345/2)*altitude/640)*
			(msg->bounding_boxes[i].ymax - msg->bounding_boxes[i].ymin)*(2*tan(1.007055/2)*altitude/480);
...
int main(int argc, char **argv)
{
	...
	{	
		...
		if(mode_g == 1) 
		{
			...
			set_destination(waypointList[counter].x, waypointList[counter].y, altitude, waypointList[counter].psi);
